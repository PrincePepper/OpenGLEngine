...
public:
    ...
    ********************************************/
    ...
    Vector3<T>
    cross(const Vector3<T> &v) { /* NOTE this function modifies the vector unlike 2D and non-member versions */
        T x_(x), y_(y), z_(z);
        x = y_ * v.z - z_ * v.y;
        y = z_ * v.x - x_ * v.z;
        z = x_ * v.y - y_ * v.x;
...
float scalar_product(const Vector3<float> &a, const Vector3<float> &rhs) {
    float scalar_product = 0;
    for (int i = 0; i < 3; ++i) {
        scalar_product += a[i] * rhs[i];
    }
